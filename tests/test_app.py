...
def test__api_put_upload__upload_txt_file__returns_400_bad_request():
    response = client.put(
        "/uploads", files={"upload": ("test.txt", b"Hello, world!", "text/plain")}
    )
...
def test__api_get_uploads__get_uploads_after_put_multiple_files__returns_200_with_ids():
    response = client.put("/uploads", files={"upload": ("test1.png", b"File 1", "image/png")})
    response.raise_for_status()
    response = client.put("/uploads", files={"upload": ("test2.jpg", b"File 2", "image/jpeg")})
    response.raise_for_status()
